    ...
    const fetchAPI = async () => {
      ...
      if (search !== undefined) {
        data = await productApi.get(
          `comment??product_id=${product_id}&comment_content=${search}&?limit=999`
        );
      }else if (searchCustomer !== undefined) {
        data = await productApi.get(
          `comment?product_id=${product_id}&customer_name=${searchCustomer}&?limit=999`
        );
      } else {
        data = await productApi.get(
          `comment?product_id=${product_id}&limit=999`
        );
      }
