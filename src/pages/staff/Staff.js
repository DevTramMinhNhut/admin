...
function Staff() {
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      ...
      if (searchName !== undefined) {
        data = await customerApi.get(`staff?staff_name=${searchName}&?limit=1000`);
      } else {
        data = await customerApi.get("staff");
      }
  ...
  const deleteStaff = (staff_id) => {
    ...
    if (agreeDelete) {
      axios.delete(`http://localhost:3000/staff/${staff_id}`);
                ...
                <tr key={index}>
                  ...
                  <td>
                    ...
                    >
                      <span className="d-inline-block">
                        <Link
                          to={`/staff/update-staff/${staff.staff_id}`}
                          className="link"
                        >
                          <Button
